...
class VisualCaptioningModel:
    def __init__(self, device: str = "cuda", model_tag: CaptionModelType = None):
        ...
        self.model_dir = DEFAULT_MODEL_CHECKPOINTS_DIR / DEFAULT_MODEL_REGISTRY.get(
            self._model_tag
        )
    ...
    def preprocess(self, input_data: Image | Path, prompt: str) -> torch.Tensor:
        if isinstance(input_data, Image) or isinstance(input_data, str):
        ...
        elif isinstance(input_data, Path) and input_data.suffix in [
            ".mp4",
            ".avi",
            ".mov",
        ]:
            ...
            raise ValueError(
                "Unsupported input type. Provide an image or a video file path."
            )
