...
def split_video_to_chunks_subprocess(
    ...
    try:
        ...
        num_segments = int(duration // chunk_duration) + (
            1 if duration % chunk_duration > 0 else 0
        )
        cache_path = (
            Path(cache_path)
            / f"video_chunks_ffmpeg_splits{chunk_duration}_{video_path.stem}"
        )
    ...
    # FIXME: solve prints to logger
        ...
        print(
            "Error: ffmpeg or ffprobe not found. Make sure they are installed and in your system's PATH."
        )
        return None
    except ValueError:
